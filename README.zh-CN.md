...
Gnalloy Pipeline 入站流控 Handler，支持 pending message/byte 边界。

该模块提供 Pipeline handler。handler 在 Channel 已存在之后观察、转换、拒绝、延迟、记录或保护消息；除非模块名明确说明，否则不拥有监听 socket 或应用协议。

## 状态

- 导入路径：`gnalloy.org/handler-flow`
- 仓库：`github.com/gnalloy/handler-flow`
- 默认分支：`dev`
- 预览安装：`go get gnalloy.org/handler-flow@dev`
- 许可证：Apache-2.0
...
本仓库负责：Gnalloy Pipeline 入站流控 Handler，支持 pending message/byte 边界。
...
- handler 只有放在正确 pipeline 位置时才可复用，位置取决于它观察的消息类型。
