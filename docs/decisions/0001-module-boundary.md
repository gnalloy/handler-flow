# ADR-001: Keep handler-flow as an Independent Gnalloy Module
...
## Decision
`handler-flow` is published as `gnalloy.org/handler-flow` and owns this responsibility: Inbound flow-control handlers for Gnalloy pipelines with bounded pending message and byte budgets.
